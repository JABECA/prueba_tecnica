...
<div class="card">
  ...
  <form method="POST" action="{{ route('carros.store') }}" class="grid">
    @csrf

    <div>
      <label for="placa">Placa</label>
      <input type="text" name="placa" id="placa" value="{{ old('placa') }}" class="form-control" style="width:100%; text-transform:uppercase" required>
      @error('placa') <small style="color:#e02424">{{ $message }}</small> @enderror
    </div>

    <div>
      <label for="color">Color</label>
      <input type="text" id="color" name="color" value="{{ old('color') }}" class="form-control" style="width:100%">
